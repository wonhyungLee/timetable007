...
- `src/App.jsx`: 전담 수업 "교환" 시에도 교사 조건으로 교환이 막히면 해결안(Plan) 자동 제안 (양쪽 전담 충돌 포함)
...
## 8) 교환 불가 자동 해결안(Plan) 기능

전담 수업을 **교환 모드**로 바꾸려 할 때도, 전담 교사 동시간대 중복(또는 교환 대상 전담의 원래 시간 중복) 때문에 1차적으로 교환이 막히는 경우가 있습니다.

이 경우에도 앱이 자동으로 **2~3단계 해결안(Plan)** 을 만들어 선택할 수 있게 제공합니다.

- 예: `A반 (월 2교시 체육)` ↔ `A반 (수 4교시 국어)` 교환이 막힘
  - `B반 (수 4교시 체육)`을 먼저 다른 시간으로 이동/교환 → 다시 `A반 체육`을 수 4교시에 교환
- 예: 교환 대상도 전담이라면(양쪽 전담) **두 전담 교사 충돌**을 동시에 해결하는 플랜을 제안합니다.

해결안은 한 번에 적용되며, 적용 직후 상단 알림의 **되돌리기(Undo)** 로 원복할 수 있습니다.
또한, 해결이 어려운 경우를 대비해 모달 마지막에 **강제 교환(빨간 테두리 표시)** 옵션도 함께 제공합니다.
